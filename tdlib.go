...
// You can provide string or UpdateData.
func (client *Client) SendAndCatch(jsonQuery interface{}, timeout time.Duration) (UpdateMsg, error) {
	var update UpdateData
...
func (client *Client) Authorize() (AuthorizationState, error) {
	...
	if state.GetAuthorizationStateEnum() == AuthorizationStateWaitEncryptionKeyType {
		ok, err := client.CheckDatabaseEncryptionKey(nil, time.Second*15)
...
func (client *Client) SendPhoneNumber(phoneNumber string, timeout time.Duration) (AuthorizationState, error) {
	...
	phoneNumberConfig.Type = "phoneNumberAuthenticationSettings"
